...
func (oq *OrderQuery) QueryShippingAddress() *AddressQuery {
...
func (oq *OrderQuery) QueryBillingAddress() *AddressQuery {
	query := (&AddressClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withShippingAddress: oq.withShippingAddress.Clone(),
...
func (oq *OrderQuery) WithShippingAddress(opts ...func(*AddressQuery)) *OrderQuery {
	query := (&AddressClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withShippingAddress = query
	return oq
}
...
func (oq *OrderQuery) WithBillingAddress(opts ...func(*AddressQuery)) *OrderQuery {
	query := (&AddressClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			oq.withItems != nil,
			oq.withPayments != nil,
			oq.withShippingAddress != nil,
			oq.withBillingAddress != nil,
		}
	)
	if oq.withUser != nil || oq.withShippingAddress != nil || oq.withBillingAddress != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oq.withShippingAddress; query != nil {
		if err := oq.loadShippingAddress(ctx, query, nodes, nil,
			func(n *Order, e *Address) { n.Edges.ShippingAddress = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withBillingAddress; query != nil {
		if err := oq.loadBillingAddress(ctx, query, nodes, nil,
			func(n *Order, e *Address) { n.Edges.BillingAddress = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadShippingAddress(ctx context.Context, query *AddressQuery, nodes []*Order, init func(*Order), assign func(*Order, *Address)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		if nodes[i].address_shipping_orders == nil {
			continue
		}
		fk := *nodes[i].address_shipping_orders
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "address_shipping_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OrderQuery) loadBillingAddress(ctx context.Context, query *AddressQuery, nodes []*Order, init func(*Order), assign func(*Order, *Address)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		if nodes[i].address_billing_orders == nil {
			continue
		}
		fk := *nodes[i].address_billing_orders
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "address_billing_orders" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
