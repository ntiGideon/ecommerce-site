...
type AddressQuery struct {
	...
	withUser           *UserQuery
	withShippingOrders *OrderQuery
	withBillingOrders  *OrderQuery
	withFKs            bool
...
// QueryShippingOrders chains the current query on the "shipping_orders" edge.
func (aq *AddressQuery) QueryShippingOrders() *OrderQuery {
...
// QueryBillingOrders chains the current query on the "billing_orders" edge.
func (aq *AddressQuery) QueryBillingOrders() *OrderQuery {
	query := (&OrderClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *AddressQuery) Clone() *AddressQuery {
	...
	return &AddressQuery{
		...
		withShippingOrders: aq.withShippingOrders.Clone(),
		withBillingOrders:  aq.withBillingOrders.Clone(),
...
// WithShippingOrders tells the query-builder to eager-load the nodes that are connected to
// the "shipping_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AddressQuery) WithShippingOrders(opts ...func(*OrderQuery)) *AddressQuery {
	query := (&OrderClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withShippingOrders = query
	return aq
}

// WithBillingOrders tells the query-builder to eager-load the nodes that are connected to
// the "billing_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AddressQuery) WithBillingOrders(opts ...func(*OrderQuery)) *AddressQuery {
	query := (&OrderClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withBillingOrders = query
	return aq
}
...
//		GroupBy(address.FieldFirstName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (aq *AddressQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Address, error) {
	var (
		...
		loadedTypes = [3]bool{
			aq.withUser != nil,
			aq.withShippingOrders != nil,
			aq.withBillingOrders != nil,
		}
	)
	if aq.withUser != nil {
		withFKs = true
	...
	if query := aq.withShippingOrders; query != nil {
		if err := aq.loadShippingOrders(ctx, query, nodes,
			func(n *Address) { n.Edges.ShippingOrders = []*Order{} },
			func(n *Address, e *Order) { n.Edges.ShippingOrders = append(n.Edges.ShippingOrders, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withBillingOrders; query != nil {
		if err := aq.loadBillingOrders(ctx, query, nodes,
			func(n *Address) { n.Edges.BillingOrders = []*Order{} },
			func(n *Address, e *Order) { n.Edges.BillingOrders = append(n.Edges.BillingOrders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AddressQuery) loadShippingOrders(ctx context.Context, query *OrderQuery, nodes []*Address, init func(*Address), assign func(*Address, *Order)) error {
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(address.ShippingOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.address_shipping_orders
		if fk == nil {
			return fmt.Errorf(`foreign-key "address_shipping_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "address_shipping_orders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AddressQuery) loadBillingOrders(ctx context.Context, query *OrderQuery, nodes []*Address, init func(*Address), assign func(*Address, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Address)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(address.BillingOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.address_billing_orders
		if fk == nil {
			return fmt.Errorf(`foreign-key "address_billing_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "address_billing_orders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
